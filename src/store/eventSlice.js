...
export const fetchEvent = (eventId, sort, pageNumber, lotStatus, username, breadcrumbs ) => (dispatch) => {
    //if eventId is not same as currentEvent's ID
    dispatch(startLoading())
    axios.get(`/event/GetEventsDetails/${eventId}/${sort}/${pageNumber}/5/${lotStatus}/${username}`)
    .then(res=>{
        dispatch(clearError())
        dispatch(setEvent({
